Shader "ATOcean/ATOceanWaterGPU"
{
    Properties
    {
        [Header(Diffuse)]
        ...
        [HideInInspector]_LengthScale1("LengthScale C1",float) = 160.0
        [Header(Cascade 2)]
        ...
        [HideInInspector]_LengthScale2("LengthScale C2",float) = 640.0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            Varyings domain(TessellationFactors factors, OutputPatch<Varyings, 3> patch, float3 barycentricCoordinates : SV_DomainLocation)
            {
                ...
                normal += SAMPLE_TEXTURE2D_LOD(
                    _Derivatives_c0, 
                    sampler_Derivatives_c0, 
                    uv0,0);

                output.normalWS = lerp( output.normalWS, normalize(normal), importance);
                output.positionWS += displacement;
                output.positionHCS.xyz -= TransformWorldToObject(displacement);
