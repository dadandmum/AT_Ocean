...
def load_data(data_dir, deltaT, max_time_shift, test_split=0.2):
    ...
    for tag in sorted(exr_files.keys()):
        ...
        for filename in sorted_files:
            ...
            try:
                ...
                # rgba_data_sequence -> [sequence_length, width * height, 6]
                # sequence_length = 图像数量
                # width * height = 像素数量
                # 6 = [r, g, b, a, u, v]
                rgbauv_data_sequence.append(np.array(rgba_uv))
        
            except Exception as e:
                print(f"Error when load file {filepath} : {e}")
        
        # 保存不同“时间序列”的数据
        # rgba_uv_dt_target_data -> [ width * height * time_shift_max, sequence_length(大约，尾帧会有剔除), 11]
        # sequence_length = 图像数量
        # 其中 11 为 [r, g, b, a, u, v, dt, target_r, target_g, target_b, target_a]
        rgba_uv_dt_target_data = []
        sequence_length = len(rgbauv_data_sequence)
        data_size = len(rgbauv_data_sequence[0])
        
        # 计算不同时间差的序列
        for timeshift in range(1, max_time_shift):
            for i in range(data_size):
                rgba_uv_dt_target = []
                # 计算时间差
                dt = timeshift * deltaT
                # 对在sequence中的不同的帧
                for j in range(sequence_length-timeshift):
                    rgba_uv_dt_target.append( list(rgbauv_data_sequence[j][i]) + [dt] + list(rgbauv_data_sequence[j+timeshift][i][:4]) )
                rgba_uv_dt_target_data.append(np.array(rgba_uv_dt_target))

        # 维度为
        if rgba_uv_dt_target_data:
            data[tag] = rgba_uv_dt_target_data
            print(f"Load Data from TAG [ {tag} ] , Total Seq: {len(rgba_uv_dt_target_data)}")

    # 根据tag 分割对应的数据队列
    train_test_data = {}
    for tag, tag_sequence in data.items():
        train_seq, test_seq = train_test_split(tag_sequence, test_size=test_split, random_state=42)
        train_test_data[tag] = (train_seq , test_seq)
...
# 3. RNN 模型定义
# --------------------------------------
class ColorPredictorRNN(nn.Module):
    def __init__(self, input_size=7, hidden_size=128, num_layers=2, rnn_type='LSTM'):
        ...
        self.fc = nn.Linear(hidden_size, 4)  # 输出 RGBA
...
# 5. 训练流程
# --------------------------------------
def train(data_dir='./data', deltaT = 0.016, max_time_shift=4, epochs=100, batch_size=16, hidden_size=128, num_layers=2, rnn_type='RNN', lr=1e-3, save_dir='./models'):
    # 确保保存目录存在
    os.makedirs(save_dir, exist_ok=True)
    
    # 加载数据
    train_test_sequences = load_data(data_dir, deltaT, max_time_shift)
    ...
    for tag, (train_seq, test_seq) in train_test_sequences.items():
        ...
        train_by_tag(tag, train_seq, test_seq, save_dir, epochs , batch_size , hidden_size , num_layers , rnn_type , lr)

    
    
def train_by_tag(tag, train_seq, test_seq, save_dir, epochs=100, batch_size=32 , hidden_size=128, num_layers=2, rnn_type='LSTM', lr=1e-3,):
    ...
    model = ColorPredictorRNN(input_size=7, hidden_size=hidden_size, num_layers=num_layers, rnn_type=rnn_type).to(device)
    ...
    for epoch in range(epochs):
        ...
        # 保存模型
        if epoch % 20 == 0:
            model_path = os.path.join(save_dir, f'{tag}_{rnn_type}_h{hidden_size}_l{num_layers}_{epoch}.pth')
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig(os.path.join(save_dir, f'{tag}_{rnn_type}_loss_curve.png'))

    plt.close()

    # 保存模型为 ONNX
    modelONNX = ColorPredictorRNN(input_size=7, hidden_size=hidden_size, num_layers=num_layers, rnn_type=rnn_type)
    modelONNX.load_state_dict(torch.load(last_model_path))
    onnx_save_path = os.path.join(save_dir, f'{tag}_{rnn_type}_model.onnx')
    save_model_as_onnx(modelONNX, onnx_save_path)

    return model

def save_model_as_onnx(model, save_path, input_size=(1, 1, 7)):
    """
    将模型保存为 ONNX 格式
    :param model: 训练好的模型
